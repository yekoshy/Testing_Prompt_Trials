[
  ...
  "*   **TC08: Maximum Length + 1 (Length 8):** Enter 8 valid characters (e.g., `aB3*X9zA`), click \"Check Input\". *Expected: \"Invalid Value\" (or the UI prevents typing the 8th character via a `maxlength` attribute).* ",
  "*   **TC09: Single Character (Length 1):** Enter 1 valid character (e.g., `A`), click \"Check Input\". *Expected: \"Invalid Value\".*",
  "*   **TC10: Extremely Long Input:** Paste a string of 50+ valid characters, click \"Check Input\". *Expected: \"Invalid Value\" (or truncated to 7 characters and validated).* ",
  "### 3. Functional Testing - Negative (Character Constraints - EP)",
  ...
  "### 5. Exploratory & Technical Testing (DOM & JS Hacking)",
  "Since the creator explicitly states this app has **deliberate bugs** and is meant for JavaScript hacking, these tests are highly relevant.",
  "*   **TC24: DOM Manipulation (Readonly):** Open Chrome DevTools, inspect the `validation_message` field, and remove the `readonly` or `disabled` attribute. Type \"Hacked\" into the field, then click \"Check Input\". *Expected: The application's JavaScript should overwrite your manual edit with the correct validation result.*",
  "*   **TC25: DOM Manipulation (Maxlength):** Inspect the DOM and change the `maxlength` attribute of the input field (if it exists) to `10`. Enter 8 valid characters and click \"Check Input\". *Expected: \"Invalid Value\".*",
  "*   **TC26: DOM Manipulation (Element ID/Name):** Inspect the DOM, change the `name` attribute of the input field from `characters` to `broken`, and click \"Check Input\". *Expected: Observe how the obfuscated JavaScript handles the missing element (does it throw a console error or fail gracefully?).*",
  "*   **TC27: JavaScript Disabled:** Disable JavaScript in your browser settings and attempt to use the form. *Expected: The button should either do nothing (since it's JS powered) or the app should display a `<noscript>` warning.*",
  "### 6. Security and Injection Tests",
  "Testing how the JavaScript handles potentially malicious inputs.",
  "*   **TC28: Cross-Site Scripting (XSS) Attempt:** Enter exactly 7 characters forming an HTML/JS payload (e.g., `<script`, `<img/>`, or `<b>x</b>`), click \"Check Input\". *Expected: \"Invalid Value\" (because `<` and `>` are not allowed), and the characters should be safely rendered without executing.*",
  "*   **TC29: SQL Injection Attempt:** Enter exactly 7 characters mimicking SQLi (e.g., `' OR 1=`), click \"Check Input\". *Expected: \"Invalid Value\".*",
  "*   **TC30: Null Byte Injection:** Attempt to paste a string containing a null byte (e.g., `abc%00def`), click \"Check Input\". *Expected: \"Invalid Value\".*",
  "### 7. Automation & Console Testing (Advanced)",
  "The creator encourages automating this app directly from the browser console.",
  "*   **TC31: ASCII Loop Test:** Write a JavaScript loop in the browser console to inject all ASCII characters (0-255) one by one into a 7-character string and trigger the button click. *Expected: Identify if any undocumented characters (like specific symbols or extended ASCII) accidentally trigger a \"Valid Value\" due to a regex bug in the obfuscated code.*"
]
